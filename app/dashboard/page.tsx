    ...
    <div data-theme="cockpit" className={`${appPageShellClass} bg-[color:var(--color-paper)] text-[color:var(--color-ink)]`}>
                    ...
                    {dueSoonAlerts.map(a => {
                      ...
                      return (
                        <button key={a.projectId} onClick={() => router.replace(`/dashboard?project=${a.projectId}`)} className="w-full text-left px-4 py-2.5 hover:bg-paper-2 transition-colors flex items-center justify-between gap-3">
                          ...
                          <span className="text-[10px] font-bold uppercase tracking-widest text-[color:var(--color-coral)] shrink-0">{timing} · NUDGE AVAILABLE</span>
                        </button>
                      );
                    })}
                  </div>
      ...
      {settlementChoice && (() => {
        ...
        const optionRowClass = (mode: TriggerMode) =>
          `flex cursor-pointer items-start gap-3 rounded-[var(--radius-box)] border p-3 ${settlementChoice.triggerMode === mode ? "border-[color:var(--color-soft)] bg-acc-soft" : "border-soft bg-[color:var(--color-paper-2)] hover:bg-[color:var(--color-paper-2)]"
          }`;
                    ...
                    <div className="relative ml-3 mt-4 border-l border-[color:var(--color-strong)] pl-5">
                      {settlementMilestones.map(milestone => {
                        ...
                        const markerClass = isSettled
                          ? "bg-[color:var(--state-success-bg)] text-[color:var(--color-grass)] border-[color:var(--color-soft)]"
                          : isCurrent
                            ? "bg-acid text-acc-ink border-acid"
                            : isCancelled
                              ? "bg-soft border-soft"
                              : "border-dashed border-[color:var(--color-strong)] text-[color:var(--color-ink-3)]";
                        const statusPillClass = isSettled
                          ...
                          : isCurrent
                            ...
                            : isCancelled
                              ...
                              : "bg-transparent text-[color:var(--color-ink-3)] border-[color:var(--color-strong)] border-dashed";
    ...
    <Suspense fallback={
      <div className={appPageShellClass}>
        ...
        <main className={`${appPageContainerClass} max-w-none px-0 py-0 flex-1 flex h-[calc(100vh-64px)] items-center justify-center`}>
          <div className="font-bold tracking-wide text-neutral-400">Loading dashboard…</div>
        </main>
      </div>
    }>
      <DashboardContent />

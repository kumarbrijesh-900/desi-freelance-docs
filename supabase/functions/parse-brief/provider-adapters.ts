...
- **Account-holder name is captured even when plain**: When the brief states the name of the account holder or payee receiving payment — a person or business, not only a UPI handle — capture it VERBATIM into payment.accountName. Cues include "account name <Name>", "payee <Name>", "pay <Name>", "transfer to <Name>", or a name stated alongside the bank/account/IFSC details. A plain payout name is as valid as a VPA; do NOT leave payment.accountName null when one is stated. This is the payout identity, distinct from agency.businessName and client.name — do not copy either into it, and never invent a name absent from the text.
- **Payment rail is verbatim, never paraphrased**: When the brief names the payment service or rail — "Wise", "Payoneer", "PayPal", "SWIFT", "NEFT", "IMPS", "RTGS", "UPI" — capture that exact word into payment.mode. NEVER generalize it to a category such as "Bank Transfer", "Online Transfer", or "International Transfer". "pay through Wise" -> payment.mode is "Wise", not a paraphrase. Only use a generic term when the brief itself is generic.

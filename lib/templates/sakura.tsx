...
export default function SakuraTemplate({ data }: InvoiceTemplateProps) {
  return (
    <div className="font-['DM_Sans',_sans-serif] text-[#2D2024] bg-white min-h-[295mm] pt-[15mm] px-[15mm] pb-[10mm] box-border relative overflow-visible print:overflow-visible print:min-h-0 print:h-auto">
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="border-b border-[#E11D48]/20">
              <th className="py-3 text-[9px] font-bold uppercase tracking-[0.2em] text-[#B09098] text-left">Description</th>
              <th className="py-3 text-[9px] font-bold uppercase tracking-[0.2em] text-[#B09098] text-center w-[50px]">Qty</th>
              <th className="py-3 text-[9px] font-bold uppercase tracking-[0.2em] text-[#B09098] text-right w-[100px]">Rate</th>
              <th className="py-3 text-[9px] font-bold uppercase tracking-[0.2em] text-[#B09098] text-right w-[120px]">Amount</th>
            </tr>
          </thead>
          <tbody>
                ...
                return (
                  <tr key={item.id} className="bg-[#FDF2F4]">
                    ...
                    <td className="py-4 px-4 text-right">
                      <p className="text-[9px] uppercase tracking-[0.15em] text-[#B09098] mb-0.5">Subtotal</p>
              ...
              return (
                <tr key={item.id} className="border-b border-[#F5E6EA]">
                  <td className="py-5 pr-6">
                    ...
                    <div className="mt-1 text-[10px] text-[#B09098] flex gap-3">
      ...
      <section className="grid grid-cols-2 gap-14 items-start">
        <div className="space-y-6">
          {data.hasBankDetails && (
            <div>
              <p className="text-[9px] uppercase tracking-[0.25em] text-[#B09098] mb-2">Bank details</p>
              <div className="text-[11px] text-[#8B7078] space-y-0.5">
                <p className="font-bold text-[#2D2024]">{data.bankName}</p>
          ...
          {data.hasNotes && (
            <div>
              <p className="text-[9px] uppercase tracking-[0.25em] text-[#B09098] mb-1">Notes</p>
              <p className="text-[10px] text-[#8B7078] leading-relaxed whitespace-pre-line max-w-[300px]">{data.notes}</p>
            </div>
          )}
          {data.hasQrCode && (
            <div>
              <p className="text-[9px] uppercase tracking-[0.25em] text-[#B09098] mb-2">Payment QR</p>
              <img src={data.qrCodeUrl} alt="QR" className="h-16 w-16 object-contain" />
            </div>
          )}
        </div>
        <div className="text-right">
          <p className="text-[9px] uppercase tracking-[0.25em] text-[#B09098] mb-5">Authorized signatory</p>
          {data.signatureUrl ? (
          ...
          <p className="text-[8px] text-[#B09098] uppercase tracking-widest">Digital signature</p>
        </div>
      </section>

...
export default function MonoTemplate({ data }: InvoiceTemplateProps) {
  return (
    <div className="font-['JetBrains_Mono',_'Fira_Code',_monospace] text-[#1a1a1a] bg-white min-h-[297mm] p-[15mm] box-border relative">
      ...
      {/* ── Client + Terms ────────────────────── */}
      <section className="grid grid-cols-[1fr_180px] gap-8 mb-10 pb-6 border-b border-dashed border-gray-300">
        <div>
        ...
        <div className="text-right">
          <p className="text-[10px] text-gray-400 uppercase tracking-widest mb-1">// terms</p>
      ...
      <section className="mb-10">
        <p className="text-[10px] text-gray-400 uppercase tracking-widest mb-4">// line_items[]</p>
        <table className="w-full border-collapse text-[11px]">
          <thead>
            <tr className="border-b-2 border-[#111]">
              <th className="py-2 text-left font-medium text-gray-400 text-[10px] uppercase tracking-wider">description</th>
              <th className="py-2 text-center w-[50px] font-medium text-gray-400 text-[10px]">qty</th>
              <th className="py-2 text-right w-[100px] font-medium text-gray-400 text-[10px]">rate</th>
              <th className="py-2 text-right w-[120px] font-medium text-gray-400 text-[10px]">amount</th>
            </tr>
          </thead>
          <tbody>
              ...
              return (
                <tr key={item.id} className="border-b border-gray-100">
                  <td className="py-3 pr-4">
                    ...
                    <div className="mt-0.5 text-[10px] text-gray-400 flex gap-3">
      ...
      <section className="flex justify-end mb-12">
        <div className="w-[280px] space-y-2">
          ...
          {data.amountInWords && (
            <p className="text-right text-[10px] text-gray-400 italic">{data.amountInWords}</p>
          )}
          {data.approximateUsd && (
            <p className="text-right text-[10px] text-gray-400">≈ {data.approximateUsd}</p>
          )}
        </div>
      </section>
        ...
        <div className="space-y-6">
          ...
          {data.hasNotes && (
            <div>
              ...
              <p className="text-[10px] text-gray-500 leading-relaxed whitespace-pre-line max-w-[300px]">{data.notes}</p>
            </div>
          )}
          {data.hasQrCode && (
            <div>
              <p className="text-[10px] text-gray-400 uppercase tracking-widest mb-2">// payment_qr</p>
              <img src={data.qrCodeUrl} alt="QR" className="h-16 w-16 object-contain" />
            </div>
          )}
        </div>
        <div className="text-right">
          <p className="text-[10px] text-gray-400 uppercase tracking-widest mb-4">// authorized</p>
          {data.signatureUrl ? (
          ...
          <p className="text-[9px] text-gray-400 uppercase tracking-widest">digital_signature</p>
        </div>
      </section>

...
export default function CoastalTemplate({ data }: InvoiceTemplateProps) {
  return (
    <div className="font-['DM_Sans',_sans-serif] text-[color:var(--color-ink)] bg-[color:var(--color-paper-2)] min-h-[295mm] tabular-nums box-border relative flex overflow-visible print:overflow-visible print:min-h-0 print:h-auto">
      ...
      <div className="flex-1 pt-[15mm] px-[15mm] pb-[10mm] pl-[12mm]">
        ...
        <header className="flex justify-between items-start mb-14">
          <div>
            ...
            {data.agencyAddress && data.agencyAddress !== "—" && (
              <p className="text-[11px] text-[color:var(--color-ink-2)] leading-relaxed whitespace-pre-line mt-1.5 max-w-[260px]">{data.agencyAddress}</p>
            )}
          ...
          <div className="text-right">
            ...
            <div className="mt-3 space-y-1.5">
              <div className="flex justify-end gap-8 text-[11px]">
                <div>
                  <p className="text-[9px] uppercase tracking-wider text-[color:var(--color-ink-2)]">Issued</p>
                  <p className="font-normal">{data.invoiceDate}</p>
                </div>
                <div>
                  <p className="text-[9px] uppercase tracking-wider text-[color:var(--color-ink-2)]">Due</p>
                  <p className="font-normal text-[color:var(--color-ochre-deep)]">{data.dueDate}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* ── Client ──────────────────────────── */}
        <section className="mb-12 pb-8 border-b border-[color:var(--color-soft)]">
          <div className="grid grid-cols-[1fr_180px] gap-10">
            ...
            <div className="text-right self-end">
              <p className="text-[9px] font-bold uppercase tracking-[0.2em] text-[color:var(--color-ink-2)] mb-1">Terms</p>
                  ...
                  return (
                    <tr key={item.id} className="bg-[#F0F9FF]">
                      ...
                      <td className="py-4 px-3 text-right">
                        <p className="text-[9px] text-[color:var(--color-ink-2)] mb-0.5">Subtotal</p>
        ...
        {/* ── Footer ──────────────────────────── */}
        <section className="grid grid-cols-2 gap-12 items-start border-t border-[color:var(--color-soft)] pt-8">
          <div className="space-y-5">
            ...
            {data.hasNotes && (
              <div>
                <p className="text-[9px] font-bold uppercase tracking-[0.2em] text-[color:var(--color-ink-2)] mb-1">Notes</p>
                <p className="text-[10px] text-[color:var(--color-ink-2)] leading-relaxed whitespace-pre-line">{data.notes}</p>
              </div>
            )}
            {data.hasQrCode && (
              <div>
                <p className="text-[9px] font-bold uppercase tracking-[0.2em] text-[color:var(--color-ink-2)] mb-2">Payment QR</p>
                <img src={data.qrCodeUrl} alt="QR" className="h-16 w-16 object-contain" />
              </div>
            )}
          </div>
          <div className="text-right">
            <p className="text-[9px] font-bold uppercase tracking-[0.2em] text-[color:var(--color-ink-2)] mb-5">Authorized signatory</p>
            {data.signatureUrl ? (

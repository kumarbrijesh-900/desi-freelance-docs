...
export default function NotificationBell() {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      ...
      default:
        return <BellIcon className="h-4 w-4 text-[color:var(--color-ink-2)]" />;
    }
  };
    ...
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-80 origin-top-right border-2 border-[#111118] bg-white shadow-[var(--brutal-shadow-lg)] z-50 overflow-hidden">
              ...
              <div className="divide-y divide-[color:var(--color-soft)]">
                {notifications.slice(0, 20).map((n) => (
                  <div
                    key={n.id}
                    className={cn(
                      "group relative flex items-start gap-3 p-4 transition-colors hover:bg-[color:var(--color-paper)]",
          ...
          <div className="border-t border-[color:var(--color-soft)] bg-[color:var(--color-paper)]/50 p-2 text-center">
            <Link
              href="/invoices"

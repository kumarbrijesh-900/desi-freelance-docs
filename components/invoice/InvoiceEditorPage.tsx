...
const handlePreviewInvoice = async () => {
  ...
  try {
    // Promote DRAFT placeholder to real INV-YYYY-NNN if user is authenticated
    let invoiceNumberForPreview = formData.meta.invoiceNumber;
    if (invoiceNumberForPreview?.startsWith("DRAFT-")) {
      const userId = await getCurrentUserId();
      if (userId) {
...
const handleSaveDraft = async () => {
  ...
  try {
    ...
    if (clientMsaNote && parserDocumentId) {
      ...
      // Promote DRAFT placeholder to real INV-YYYY-NNN if user is now authenticated
      let formDataForSave = formData;
      if (formData.meta.invoiceNumber?.startsWith("DRAFT-")) {
        const { generateNextInvoiceNumber } = await import("@/lib/supabase/invoices");
        const realNumber = await generateNextInvoiceNumber();
        if (realNumber) {
          formDataForSave = {

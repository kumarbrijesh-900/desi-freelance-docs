...
import { INTERNATIONAL_COUNTRY_OPTIONS, INTERNATIONAL_CURRENCY_OPTIONS } from "@/lib/international-billing-options";
...
function isMandatoryField(label: string, data: InvoiceFormData): boolean {
  ...
  if (l.includes("gstin") && !l.includes("client") && data.agency.gstRegistrationStatus === "registered") return true;
  if (l.includes("pan") && data.agency.gstRegistrationStatus !== "registered") return true;
  if (l.includes("bank name") || l.includes("account number") || l.includes("ifsc")) return true;
...
function getExtractedValueForLabel(label: string, data: InvoiceFormData): string {
  ...
  if (l.includes("agency registered") || l.includes("gst status")) return String(data.agency.gstRegistrationStatus === "registered");
...
function setFormDataValue(data: InvoiceFormData, label: string, value: string): InvoiceFormData {
  ...
  if (l.includes("gst status") || l.includes("registered")) next.agency.gstRegistrationStatus = value === "true" ? "registered" : "not-registered";
  ...
  if (l.includes("client gstin") || (l.includes("gstin") && l.includes("client"))) next.client.clientGstin = value;
  if (l.includes("currency")) next.client.clientCurrency = value as any;

  if (l.includes("description") || l.includes("deliverable")) {
    if (!next.lineItems[0]) next.lineItems[0] = { id: "line-1", description: "", qty: 1, rate: 0, type: "Other", rateUnit: "per-deliverable" };
...
function getFieldType(label: string): "text" | "select" | "date" | "checkbox" {
  const l = label.toLowerCase();
  if (l.includes("state") || l.includes("country") || l.includes("currency")) return "select";
  if (l.includes("date")) return "date";
  if (l.includes("registered") || l.includes("status") || l.includes("available") || l.includes("lut")) return "checkbox";
  return "text";
}
...
function getSelectOptions(label: string): string[] {
  ...
  if (l.includes("currency")) return ["INR", ...INTERNATIONAL_CURRENCY_OPTIONS.map(c => c.code)];
  return [];
}
    ...
    <div className={cn(
      ...
      <div className="flex items-center justify-between">
        <span className={cn(
          ...
          isApproved ? "text-[color:var(--color-lime-400)]" : isLowConfidence ? "text-amber-500" : isMandatory ? "text-cyan-500" : "text-gray-500"
        )}>
          ...
          {fieldType === "checkbox" ? (
            ...
            >
              ...
              <div className={cn(
                ...
                editValue === "true" ? "bg-[color:var(--color-lime-500)] border-transparent" : "bg-white/5 border-white/10"
              )}>
                {editValue === "true" && <CheckIcon className="h-3 w-3 text-black" />}
              </div>
            </button>
          ) : fieldType === "select" ? (
            <select
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 p-4 backdrop-blur-md">
        <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }} className="flex max-h-[92vh] w-full max-w-2xl flex-col overflow-hidden rounded-[28px] bg-[#0c0c12] shadow-2xl border border-white/[0.1]">
          ...
          <div className="border-t border-white/[0.08] bg-black/40 p-6 flex items-center justify-between gap-4">
            <button onClick={onParseAgain} className="px-5 py-2.5 text-sm font-bold text-gray-400 hover:text-white transition-colors">
              Parse Again
            </button>
            <div className="flex items-center gap-3">
              <button onClick={() => onSubmit(localData, shouldSaveClient)} disabled={!allReviewed} className={cn("px-8 py-3 rounded-2xl text-sm font-black transition-all shadow-lg active:scale-95", allReviewed ? "bg-[color:var(--color-lime-500)] text-black hover:bg-[color:var(--color-lime-400)] shadow-lime-500/20" : "bg-white/[0.05] text-white/20 cursor-not-allowed")}>
                Generate Invoice
              </button>
            </div>
          </div>

...
export function ActiveDrilldown({
  ...
  if (!state || !state.invoice) {
    return (
      <div className="px-6 py-5 border-b border-soft bg-paper-2">
        ...
        <div className="type-title font-bold text-ink/50">
  ...
  if (invoice?.due_date && dueDays !== null) {
    ...
    if (dueDays > 0) {
      countdownJSX = <div className="type-label font-bold uppercase tracking-widest text-ink/70 mt-1">DUE IN {dueDays} DAYS · {shortDate}</div>;
    } else if (dueDays === 0) {
      countdownJSX = <div className="type-label font-bold uppercase tracking-widest text-[color:var(--color-coral)] mt-1">DUE TODAY</div>;
    } else {
      countdownJSX = <div className="type-label font-bold uppercase tracking-widest text-[color:var(--color-coral)] mt-1">OVERDUE BY {Math.abs(dueDays)} DAYS</div>;
    }
  }

  return (
    <div className="flex flex-col xl:flex-row gap-5 min-h-0">
      <div className="flex-1 bg-paper rounded-[var(--radius-box)] p-5 border border-soft flex flex-col">
        <div className="flex justify-between items-start mb-5">
          <div>
            <div className="flex gap-2 mb-2">
              <div className="px-2 py-0.5 bg-acid text-acc-ink type-label font-bold uppercase tracking-widest border border-soft rounded-full shadow-[var(--brutal-shadow-sm)] flex items-center gap-1.5"><span className="w-1.5 h-1.5 bg-acc-ink rounded-full animate-pulse" /> ACTIVE NOW</div>
            </div>
            <h2 className="type-display font-display font-bold tracking-tight leading-tight mb-1.5 text-ink">{title}</h2>
            <h3 className="type-label font-bold tracking-widest text-ink/70 uppercase">{subtitle}</h3>
          </div>
        ...
        <div className="type-label font-bold uppercase tracking-widest text-ink/70 mb-3">LINE ITEMS · {items.length}</div>
            ...
            return (
              <div key={idx} className={`flex justify-between items-center py-2 ${idx < items.length - 1 ? 'border-b border-ink/20 border-dashed' : ''}`}>
                <div className="flex items-center gap-2.5 flex-1">
                  ...
                  <div className="type-body font-bold text-ink">{item.description || item.name || "Item"}</div>
                </div>
                <div className="flex items-center gap-8">
                  <div className="type-label font-bold tracking-widest text-ink/70">
                    {item.qty} {item.rateUnit === 'flat' ? '' : '×'} {item.rateUnit === 'flat' ? '' : formatInr(item.rate || 0)}
                  </div>
                  <div className="w-[90px] text-right type-body font-bold text-ink">
        ...
        <div className="border-t border-soft mt-auto pt-4 flex justify-between items-end">
          <div className="flex flex-col items-start gap-0.5">
            <div className="type-label font-bold uppercase tracking-widest text-ink/70">
              {milestone ? `M${(milestone.order_index ?? 0) + 1} TOTAL` : 'TOTAL'} · {taxBreakdown.label}
            </div>
            <div className="type-heading font-bold text-ink leading-none mt-1">
          ...
          <div className="flex flex-col items-end gap-2">
            ...
            {primary_action === 'mark_settled' && state?.next_milestone && (
              <div className="type-label font-bold uppercase tracking-widest text-ink/50 text-right mt-1">
                {(() => {
      ...
      <div className="w-full xl:w-[280px] flex flex-col gap-3">
        <div className="type-label font-bold uppercase tracking-widest text-ink mb-1">ACTIVITY</div>
        {loadingActivities ? (
          <div className="type-body font-bold text-ink/50 py-2">Loading...</div>
        ) : activities.length === 0 ? (
          <div className="type-body font-bold text-ink/50 py-2">No activity yet</div>
        ) : (
          activities.map((act) => {
            let ic = "·";
            let k = "paper";
            ...
            return (
              <div key={act.id} className="flex items-start gap-2.5" title={act.message || ""}>
                <div className={`w-7 h-7 flex items-center justify-center border border-ink font-bold type-body shrink-0 ${bgClass}`}>
                  {ic}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="type-body font-bold text-ink leading-tight mt-0.5 truncate" title={act.title}>
                    {act.title}
                  </div>
                  <div className="type-label font-bold uppercase tracking-widest text-ink/50 mt-0.5 truncate">

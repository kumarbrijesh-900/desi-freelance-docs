  ...
  const bar = (
    <div className={barClass}>
      {back && (
        <Link
          href={back}
          className="is-interactive -ml-2 inline-flex shrink-0 items-center gap-1 rounded-[var(--radius-pill)] px-2 py-1 font-mono text-[10px] uppercase tracking-[0.08em] text-ink-2 transition-colors hover:text-ink"
        >
      ...
      <TitleTag className="min-w-0 truncate font-display text-[15px] font-bold leading-none text-ink">
        {title}
      </TitleTag>

      {meta && (
        <span className="hidden min-w-0 truncate font-mono text-[10px] uppercase tracking-[0.1em] text-ink-3 sm:block">
          {meta}
        </span>
      )}

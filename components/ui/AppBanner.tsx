...
export function AppBanner({ tone = "default", className, children, ...props }: AppBannerProps) {
  return (
    <div
      className={cn(
        "w-full border-b-2 border-soft px-4 py-3 text-[length:var(--text-body)] font-bold uppercase tracking-wider text-center",
        tone === "default" && "bg-[color:var(--color-acc-soft)] text-[color:var(--color-ink)]",
        ...
        tone === "danger" && "bg-[color:var(--color-coral-400)] text-[color:var(--color-ink)]",
        className
      )}
      {...props}
    >

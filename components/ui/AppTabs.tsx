...
export const AppTabsList = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        "inline-flex h-[var(--control-lg)] items-center justify-center border-b-2 border-soft bg-paper-2 text-[color:var(--color-ink-2)]",
        className
      )}
      {...props}
    />
...
>(({ className, active, ...props }, ref) => (
  <button
    ref={ref}
    className={cn(
      "inline-flex h-full items-center justify-center whitespace-nowrap px-6 py-1.5 type-body font-bold uppercase tracking-wider transition-all disabled:pointer-events-none disabled:opacity-50",
